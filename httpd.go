...
func startHttpd() bool {
	...
	mux.Handle("/view/", http.StripPrefix("/view/", http.FileServer(http.FS(uiFS))))
	// 状態取得API(ヘッダステータス)
	...
	// 通信速度履歴取得AP(Statsタブ)
	mux.HandleFunc("/api/traffic-history", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
	...
	mux.HandleFunc("/api/read-config", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		if !ReloadCfg() {
	...
	// Proxy中継ログレベル取得API
	mux.HandleFunc("/api/get-log-level", func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		json.NewEncoder(w).Encode(map[string]int{"level": LogLevel})
	})
	// Proxy中継ログレベル変更API
	mux.HandleFunc("/api/set-log-level", func(w http.ResponseWriter, r *http.Request) {
	...
	// 起動
	httpdServer = &http.Server{Addr: ":" + strconv.Itoa(CurrentHttpdPort), Handler: mux}
	IsHttpdRun = true
	go func() {
		notify("INFO", "Httpd", "Started. (Port:"+strconv.Itoa(CurrentHttpdPort)+")")
